...
check(baseline.length === 5, 'contract keeps five independently named version checks')
check(baseline.every(({ ok }) => ok), 'desktop 1.4.3 is valid while the mobile candidate remains 1.4.2')
check(
...
const cases = [
  ['malformed desktop version fails closed', { desktopVersion: 'release-next' }, ['Desktop package version is a valid release version']],
