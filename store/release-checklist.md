...
- [ ] `npm run mobile:store:check:live` passes with zero failures.
  - 🔴 **Fresh Apple-mode probe 2026-08-12: 34 passed, 1 warning, 1 failed.** Production is healthy at commit `1183459`, stage `full`, with account-recovery email and Apple IAP capabilities true, but `/health` still reports API version `1.4.1` while the store candidate is `1.4.2`. Configuration readiness is not artifact/version parity; the live gate correctly remains open. Real email delivery and purchase behavior are also separate unproven checks.
  - 🔧 **Root cause fixed locally, not deployed.** The server package and lockfile were bumped to 1.4.2 and the static readiness gate now prevents this fifth version surface from drifting unnoticed. A future approved commit/push/redeploy must occur before rerunning the live gate.
